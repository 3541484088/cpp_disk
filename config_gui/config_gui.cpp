...
ConfigGui::ConfigGui(QWidget *parent)
    : QWidget(parent)
{
    std::cout << "[DEBUG] ConfigGui constructor started" << std::endl;
    ui.setupUi(this);
    std::cout << "[DEBUG] ui.setupUi done" << std::endl;
    config_gui = this;

    //删除原窗口边框
    setWindowFlags(Qt::FramelessWindowHint);
    std::cout << "[DEBUG] setWindowFlags done" << std::endl;
    ...
    std::cout << "[DEBUG] Server: " << server_ip << ":" << server_port << std::endl;

    MCONF->set_server_ip(server_ip.c_str());
    MCONF->set_server_port(server_port);
    MCONF->set_auto_connect(true);
    std::cout << "[DEBUG] MCONF settings done" << std::endl;

    MCONF->StartConnect();
    std::cout << "[DEBUG] MCONF->StartConnect() done" << std::endl;
    ...
    std::cout << "[DEBUG] XAuthClient->StartConnect() done" << std::endl;
  ...
  /*  XAuthClient::Get()->LoginReq("root", "123456");
    ...
    std::cout << "[DEBUG] MCONF->LoadConfigResCB set" << std::endl;
    Refresh();
    std::cout << "[DEBUG] Refresh() done" << std::endl;
...
{
    std::cout << "[DEBUG] AddConf() called" << std::endl;
    //打开配置编辑窗口，等待返回
    ConfigEdit edit;
    std::cout << "[DEBUG] ConfigEdit created" << std::endl;
    if (edit.exec() == QDialog::Accepted)
    {
        AddLog("添加配置成功");
    }
...
bool ConfigGui::CheckLogin(std::string ip, int port)
{
    ...
    std::cout << "[DEBUG] CheckLogin(" << ip << ", " << port << ") - is_login=" << is_login << ", last_ip=" << last_ip << ", last_port=" << last_port << std::endl;
    ...
    XAUTH->set_server_port(AUTH_PORT);
    std::cout << "[DEBUG] CheckLogin - XAUTH connecting to " << ip << ":" << AUTH_PORT << std::endl;
    XAUTH->Close();
    if (!XAUTH->AutoConnect(1))
    {
        AddLog("验证服务连接失败");
        std::cout << "[DEBUG] CheckLogin - XAUTH->AutoConnect failed" << std::endl;
        return false;
    }
    std::cout << "[DEBUG] CheckLogin - XAUTH->AutoConnect succeeded" << std::endl;

    LoginGUI login;
    std::cout << "[DEBUG] CheckLogin - LoginGUI created, calling exec()" << std::endl;
    int result = login.exec();
    std::cout << "[DEBUG] CheckLogin - LoginGUI exec returned: " << (result == QDialog::Accepted ? "Accepted" : "Rejected") << std::endl;
    if (result == QDialog::Rejected)
    {
        std::cout << "[DEBUG] CheckLogin - Login rejected, returning false" << std::endl;
        return false;
    }
    is_login = true;
    std::cout << "[DEBUG] CheckLogin - Login successful, returning true" << std::endl;
    return true;
}
...
{
    std::cout << "[DEBUG] Refresh() started" << std::endl;
    while (ui.tableWidget->rowCount() > 0)
        ui.tableWidget->removeRow(0);
    // 重新连接，如果修改配置中心的IP或者端口
    string server_ip = ui.server_ip_edit->text().toStdString();
    int server_port = ui.server_port_box->value();
    std::cout << "[DEBUG] Refresh - Server: " << server_ip << ":" << server_port << std::endl;

    if (!CheckLogin(server_ip, server_port))
    {
        std::cout << "[DEBUG] CheckLogin failed" << std::endl;
        AddLog("验证登录失败");
        return;
    }
    std::cout << "[DEBUG] CheckLogin succeeded" << std::endl;
    // 整理历史列表
    AddLog("整理历史列表");
    while (ui.tableWidget->rowCount() > 0)
        ui.tableWidget->removeRow(0);
